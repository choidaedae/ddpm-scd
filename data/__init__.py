...
#Create SCD dataloader
def create_scd_dataloader(dataset, dataset_opt, phase):
    '''create dataloader '''
    if phase == 'train' or 'val' or 'test':
        return torch.utils.data.DataLoader(
            dataset,
...
#Create semantic-change-detection dataset 
def create_scd_dataset(dataset_opt, phase):
    '''create dataset'''
    mode = dataset_opt['mode']
    from data.SCDDataset import SCDDataset as D
    dataset = D(dataroot=dataset_opt['dataroot'],
                resolution=dataset_opt['resolution'],
                split=phase,
                data_len=dataset_opt['data_len']
