...
def train(args):
    ...
    features, labels = prepare_data(args) # 여기서 문제 발생 
    ...
    for MODEL_NUMBER in range(args['start_model_num'], args['model_num'], 1):
        ...
        classifier = nn.DataParallel(classifier, device_ids=[0]).cuda()
        ...
        best_loss = 10000000
        stop_sign = 0
        for epoch in range(10000): # default: 100 epoch 
            for X_batch, y_batch in train_loader: # train_loader의 길이 출력하는 법? 
                X_batch, y_batch = X_batch.to(dev()), y_batch.to(dev())
                y_batch = y_batch.type(torch.long)
                
                optimizer.zero_grad()
                y_pred = classifier(X_batch)
                loss = criterion(y_pred, y_batch) # 여기서 문제 생김
...
if __name__ == '__main__':
    ...
    if not all(pretrained): # pretrained pixel classifier가 있다면 evaluation, 없다면 train 
        # train all remaining models
        opts['start_model_num'] = sum(pretrained) # 10번 학습
        train(opts)
    
    print('Loading pretrained models...') 
    models = load_ensemble(opts, device='cuda') # 10개 모델 앙상블
