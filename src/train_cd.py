# code for change detection network 
# It needs to pretrained DDPM weight 
...
if __name__ == "__main__":
    ...
    parser.add_argument('-c', '--config', type=str, default='config/CD/cd_train.json',
    ...
    # train 
    if opt['phase'] == 'train': 
        for current_epoch in range(start_epoch, n_epoch): # n_epoch = 120  
            ...
            logger.info(message)
            for current_step, train_data in enumerate(train_loader): # 여기 안에 batch_size가 있음 
                ...
                for t in opt['model_cd']['t']:
                    fe_A_t, fd_A_t, fe_B_t, fd_B_t = diffusion.get_feats(t=t) #np.random.randint(low=2, high=8)  # 여기서 memory 많이 사용함. 
                    if opt['model_cd']['feat_type'] == "dec":
                        f_A.append(fd_A_t)
                        f_B.append(fd_B_t)
                        # Uncomment the following line to visualize features from the diffusion model
