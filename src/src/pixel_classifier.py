...
def predict_labels(models, features, size): # segmentation하는 부분 
    ...
    softmax_f = nn.Softmax(dim=1) # 픽셀별로 분류할 때 softmax 사용 
    with torch.no_grad():
        for MODEL_NUMBER in range(len(models)):
            preds = models[MODEL_NUMBER](features.cuda()) # 65536, 7 
...
def load_ensemble(args, model_list, device='cpu'):
    models = []
    for i in range(args['model_num']):
        model_path = os.path.join(args['exp_dir'], model_list[i])
        print('test with', model_list[i])
