...
if __name__ == "__main__":
    ...
    optimizer = torch.optim.SGD(filter(lambda p: p.requires_grad==False, params.values()), lr=opt['model_scd']['lr'], \
        ...
        if current_epoch % opt['train']['val_freq'] == 0:
            ...
            for current_epoch in range(start_epoch, n_epoch):         
                ...
                # Feed data to diffusion model
                    ...
                    f_A = diffusion.netG.module.feats_for_pc(val_data['A'])
                    f_B = diffusion.netG.module.feats_for_pc(val_data['B'])
            ...
            change_detection.save_network(current_epoch, is_best_model = (cd_best_model or scd_best_model)) 
            change_detection._clear_cache()
            
            semantic_segmentation.save_network(current_epoch, is_best_model = (seg_best_model or scd_best_model))
            semantic_segmentation._clear_cache()
