...
class SS(BaseModel):
    ...
    def get_feature(self):
        ...
        self.feats_B = self.collect_features(self.dim, self.feats_B) # return type은 (b, c, h, w) 
    ...
    def get_current_visuals(self):
        ...
        out_dict['pred_ss_A'] = torch.Tensor(self.pred_ss_A)
        out_dict['pred_ss_B'] = torch.Tensor(self.pred_ss_B)
        out_dict['gt_cm_A'] = self.data["L1"]
        out_dict['gt_cm_B'] = self.data["L2"]
        return out_dict
    ...
    def load_network(self):
        ...
        if load_path is not None:
            ...
            if self.opt['phase'] == 'train':
                ...
                self.optSS.load_state_dict(opt['optimizer'])
    ...
    def _update_metric(self):
        ...
        """
        # compute mean IoU 
        # semantic segmentation이니까 change map에 포함된 부분에 대한 mIoU 구하기? 
    ...
    def _update_ss_metrics(self): # preds랑 labels 파악하기
        preds = []
        labels = [] # pred_ss: (b, 256x256, 8) / # labels: (b, 3, 256, 256) 
        # 둘 다 index map (b, 256, 256) 으로 바꿔야 함. 
        pred_ss_A = torch.argmax(self.pred_ss_A, dim = 2).reshape(-1, self.dim[0], self.dim[1])
        pred_ss_B = torch.argmax(self.pred_ss_A, dim = 2).reshape(-1, self.dim[0], self.dim[1])
        labels_A  = self.get_label(self.data['L1'])
        labels_B  = self.get_label(self.data['L2'])
        ...
        for pred, label in zip(preds, labels):
            ...
            hist += self.get_hist(infer_array, label_array, self.num_class)
    ...
    def pixeltolabelMap(self):
        labelMap_A = torch.argmax(self.pred_ss_A, dim=2, keepdim=False).reshape(-1, self.dim[0], self.dim[1]).cpu().numpy()
        labelMap_B = torch.argmax(self.pred_ss_B, dim=2, keepdim=False).reshape(-1, self.dim[0], self.dim[1]).cpu().numpy()
        
        return (labelMap_A, labelMap_B)
