...
if __name__ == "__main__":
    ...
    for current_step, test_data in enumerate(test_loader): # 이 때 test_data는 semantic change detection 용도의 data. segmentation과 change_detectio에 같은 데이터셋을 씀 
        ...
        diffusion.netG.set_feature_extractor(False, opt['model_ss']['steps'], opt['model_ss']['blocks'] )
        f_A = diffusion.netG.feats_for_pc(test_data['A'])
        f_B = diffusion.netG.feats_for_pc(test_data['B'])
