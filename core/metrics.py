...
def make_SCD_map(img1, img2, cd):
    ...
    cd_neg = cd.copy()
    cd_neg = cd_neg.astype(dtype = np.uint8)
    cd_neg = (255 - cd_neg) # 이미지 반전 

    scd_A = np.zeros_like(img1)
    scd_B = np.zeros_like(img2)
    # image read 
    ...
    scd_A_tensor = F.to_tensor(scd_A) # (518, 518, 3)
    scd_B_tensor = F.to_tensor(scd_B) # (518, 518, 3)
